# Apache licenced copy from https://github.com/krobertson/metis/blob/master/lib/metis/nrpe_packet.rb
...
class NrpePacket
  ...
  def self.read(data)
    values = data.unpack("nnNnA#{MAX_PACKETBUFFER_LENGTH}n")
    packet = self.new(values)
    packet.validate_crc32
    packet
  end

  def self.to_response(result, message)
